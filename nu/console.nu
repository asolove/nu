...
(function make-scrollview (frame block)
     (let (v ((NSScrollView alloc) initWithFrame: frame))
          (v set: (autoresizingMask: 18 hasHorizontalScroller: NO hasVerticalScroller: NO borderType: 2))
          (block v)
          v))
...
(class NSTextStorage
     ...
     (class NuConsoleViewController is NSObject
          ...
          (imethod (void) write: (id) string is
               ...
               (set @startOfInput (+ @startfOInput (string length)))
